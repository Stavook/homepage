...
export default function Header() {
  ...
  return (
    <header className="z-[999] fixed top-0 left-0 w-full">
      <div
        className="absolute top-0 left-0 w-full h-16 bg-[#ffffff40] dark:bg-[#20202380] backdrop-blur-md"
      ></div>
      ...
      <ul className="flex w-full max-w-md md:max-w-full flex-wrap items-center gap-2 text-md font-medium sm:flex-nowrap">
          {links.map((link) => (
            ...
            >
              ...
              >
                {link.name === "Stavros Karadimitriou" && <FaFrog size={22} />}
                {link.name}
                {link.name === activeSection && (
                  <motion.span
                    className='bg-violet-500 h-1 w-full absolute bottom-0 left-0'
